...
import com.proathome.ServicioProfesional;
...
public class ServicioTaskSesionDisponible extends AsyncTask<Void, Void, String> {

    public ServicioTaskSesionDisponible(Context contexto, int idSesion, int idPerfil, int tipoPerfil, FragmentActivity activity){
                ...
                if (Constants.tipoPerfil_DISPONIBILIDAD_PROGRESO == DetallesFragment.CLIENTE) {//Si el tipo de PERFIL es = CLIENTE
                        ...
                        if(Constants.dispProfesional_DISPONIBILIDAD_PROGRESO){//Si el profesional está en conexión.
                            ...
                            if (Constants.estatus_DISPONIBILIDAD_PROGRESO == Constants.ESTATUS_ENPAUSA_TE) {//Si el estatus de la SERVICIO - TE = PAUSA.
                                ...
                                if(ServicioCliente.terminado_TE){
                                    ...
                                    ServicioTaskFinalizarServicio finalizarServicio = new ServicioTaskFinalizarServicio(Constants.contexto_DISPONIBILIDAD_PROGRESO, Constants.idSesion_DISPONIBILIDAD_PROGRESO, Constants.idPerfil_DISPONIBILIDAD_PROGRESO, Constants.FINALIZAR_SERVICIO, DetallesFragment.CLIENTE);
                    ...
                    if(Constants.TE_activado_DISPONIBILIDAD_PROGRESO){//Si tenemos tiempo en TE (Si estamos en tiempo extra)
                        ...
                        if (Constants.progresoSegundosTE_DISPONIBILIDAD_PROGRESO <= 1 && Constants.progresoTE_DISPONIBILIDAD_PROGRESO < 1) {
                            ServicioTaskCambiarEstatusServicio cambiarEstatusServicio = new ServicioTaskCambiarEstatusServicio(Constants.contexto_DISPONIBILIDAD_PROGRESO, Constants.idSesion_DISPONIBILIDAD_PROGRESO, Constants.idPerfil_DISPONIBILIDAD_PROGRESO, Constants.tipoPerfil_DISPONIBILIDAD_PROGRESO, Constants.ESTATUS_TERMINADO_TE);
                            cambiarEstatusServicio.execute();
                        ...
                        if (Constants.progresoSegundos_DISPONIBILIDAD_PROGRESO <= 1 && Constants.progreso_DISPONIBILIDAD_PROGRESO < 1) {
                            ServicioTaskCambiarEstatusServicio cambiarEstatusServicio = new ServicioTaskCambiarEstatusServicio(Constants.contexto_DISPONIBILIDAD_PROGRESO, Constants.idSesion_DISPONIBILIDAD_PROGRESO, Constants.idPerfil_DISPONIBILIDAD_PROGRESO, Constants.tipoPerfil_DISPONIBILIDAD_PROGRESO, Constants.ESTATUS_TERMINADO);
                            cambiarEstatusServicio.execute();
                            ServicioProfesional.timer.cancel();
                            ServicioProfesional.pausa_start.setVisibility(View.INVISIBLE);
                            ServicioProfesional.terminar.setVisibility(View.VISIBLE);
